import { listSportActivities } from '../services/sportApi.js?v=2';
  ...
  render() {
    ...
    this.element.innerHTML = `
      ...
      <div class="card"><div class="card-body"><div class="table-container"><table class="data-table"><thead><tr><th>Date</th><th>Sport</th><th>Distance</th><th>Durée</th><th>D+</th><th>Source</th></tr></thead><tbody>
        ${this.loading ? '<tr class="table-loading" role="status"><td colspan="6">Chargement des activités...</td></tr>' : this.error ? '<tr class="table-error" role="alert"><td colspan="6">Impossible de charger les activités.</td></tr>' : this.items.length ? this.items.map(item => `<tr><td>${new Date(item.started_at).toLocaleString('fr-FR')}</td><td>${this._escape(item.activity_name || item.sport_type)}</td><td>${item.distance_m == null ? '-' : `${(item.distance_m / 1000).toFixed(2)} km`}</td><td>${this._duration(item.duration_seconds)}</td><td>${item.elevation_gain_m == null ? '-' : `${Math.round(item.elevation_gain_m)} m`}</td><td>${this._escape(item.source_type)}</td></tr>`).join('') : '<tr><td colspan="6">Aucune activité.</td></tr>'}
